<template>
  <el-table v-loading="loading" :data="list">
    ...
    <el-table-column label="开始日期" align="center" prop="startDate" :formatter="dateFormatter2"/>
    <el-table-column label="开始节次" align="center" prop="startTimeSlot">
      <template #default="scope">
...
import {HolidayRuleApi} from "@/api/school/rule/holidayRule";


defineOptions({ name: 'HolidayRuleList' })
...
const getList = async () => {
  loading.value = true
  try {
    const data = await HolidayRuleApi.getHolidayRulePage(queryParams)
...
const handleDelete = async (id: number) => {
  try {
    ...
    // 发起删除
    await HolidayRuleApi.deleteHolidayRule(id)
